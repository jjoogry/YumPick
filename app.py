...
def get_menu_candidates(weather_main, temp):
    ...
    if "Rain" in weather_main or "Drizzle" in weather_main:
        menus = ["해물파전", "바지락 칼국수", "짬뽕", "우동", "김치전", "수제비"]
        comment = "☔ 비랑 어울리는 메뉴는 어때요?"
    elif "Snow" in weather_main:
        menus = ["군고구마", "어묵탕", "붕어빵", "만두전골", "설렁탕", "호빵", "우동"]
        ...
        if temp <= 0:
            ...
            menus = ["온메밀", "잔치국수", "일본 라멘", "베트남 쌀국수", "샤브샤브", "수제비", "츠케멘", "떡라면"]
            comment = "🌬️ 쌀쌀한 날씨엔 따뜻한 면 요리 어때요?"
        elif 10 < temp <= 22:
            menus = ["한강 라면", "샌드위치", "수제버거", "떡볶이", "김밥", "토스트", "도시락", "샐러드"]
            comment = "✨ 날씨도 좋은데 피크닉 어때요?"
        elif 22 < temp < 30:
            menus = ["초밥", "치킨", "피자", "연어덮밥", "파스타", "타코", "비빔면", "돈까스", "화덕피자"]
